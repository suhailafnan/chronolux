...
const AddProductTo = async (req, res) => {
  try {
      const { tax_rate, stock, price, product_name, Full_description, category } = req.body;
      ...
      if (errors.length > 0) {
          return res.render("page_add_products", { categories: categories, errors: errors });
      }
      ...
      await products.save();
      res.redirect("/admin/products_list?message=Product added successfully&messageType=success");
  } catch (error) {
      console.error('Error:', error);
      res.redirect("/admin/products_list?message=Failed to add product&messageType=error");
  }
};
...
const loadProductList = async (req, res) => {
  ...
  const updateProduct = async (req, res) => {
    try {
      ...
      if (!product_id) {
        return res.redirect("/admin/products_list?message=Product ID is missing&messageType=error");
      }  
      const product = await Products.findById(product_id);
      if (!product) {
        return res.redirect("/admin/products_list?message=Product not found&messageType=error");
      } 
      ...
      if (errors.length > 0) {
        return res.redirect(`/admin/products_list?message=${encodeURIComponent(errors.join(', '))}&messageType=error`);
      }
    
     const categoryDoc = await Category.findById(category);
      if (!categoryDoc) {
        
        return res.redirect("/admin/products_list?message=Category not found&messageType=error");
      }
      ...
      res.redirect("/admin/products_list?message=Product updated successfully&messageType=success");
    } catch (error) {
      console.error('Error:', error.message);
      res.redirect(`/admin/products_list?message=${encodeURIComponent(error.message)}&messageType=error`);
    }
  };
